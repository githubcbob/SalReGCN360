...
class Multi_Heads_NL_Attention(nn.Module):
    ...
    def __init__(self, in_dim, num_heads=8, kv_stride=2):
        ...
        self.key_conv = nn.Conv1d(
            ...
            kernel_size=1,
            bias=False)
        self.key_conv.kaiming_init = False
        ...
        self.value_conv = nn.Conv1d(
            ...
            kernel_size=1,
            bias=False)
        self.value_conv.kaiming_init = False
        ...
        self.proj_conv = nn.Conv1d(
            ...
            kernel_size=1,
            bias=True)
        self.proj_conv.kaiming_init = False
        self.gamma = nn.Parameter(torch.zeros(1))

        self.q_downsample = None

        if self.kv_stride > 1:
            self.kv_downsample = nn.AvgPool1d(
...
#        _, _, h_kv, w_kv = x_kv.shape
        _, _, node_kv = x_kv.shape

        proj_key = self.key_conv(x_kv).view(
            (n, num_heads, self.qk_embed_dim, node_kv))
        ...
        energy = torch.matmul(appr_bias, proj_key).\
            view(n, num_heads, 1, node_kv)
        
        attention = F.softmax(energy, 3)

        proj_value = self.value_conv(x_kv)
        proj_value_reshape = proj_value.\
            view((n, num_heads, self.v_dim, node_kv)).\
            permute(0, 1, 3, 2)

        out = torch.matmul(attention, proj_value_reshape).\
            permute(0, 1, 3, 2).\
            contiguous().\
            view(n, self.v_dim * self.num_heads, 1)
